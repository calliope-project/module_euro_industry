...
def transport_equipment(idees,config,projection_year,output_file):
    ...
    for carrier,data in config["steam_processing"]["technology"].items():
        if data["shares"][projection_year] != 0:
            df.loc[carrier,sector] += s_ued["Trans. Eq.: Steam processing"]/data["efficiency"] * data["shares"][projection_year]
