...
def other_industrial_sectors(idees, config, projection_year, output_file):
    ...
    for carrier,data in config["steam_processing"]["technology"].items():
        if data["shares"][projection_year] != 0:
            df.loc[carrier,sector] += s_ued["Other Industrial sectors: Steam processing"]/data["efficiency"] * data["shares"][projection_year]
